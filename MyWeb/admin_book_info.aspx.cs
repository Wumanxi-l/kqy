...
public partial class admin_book_info : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            dt_ComInfo = BLL.Admin_Bll.Get_Per();
            ...
            dp_type.DataSource = BLL.Admin_Bll.Get_booktype();
            dp_type.DataTextField = "CateName";
            dp_type.DataValueField = "CategoryID";
            dp_type.DataBind();
            dp_type.Items.Insert(0, new ListItem("---请选择---", "0"));
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        for (int i = 0; i < Repeater1.Items.Count; i++)
        {
            ...
            if (checkbox.Checked == true)                   //是否被选中
            {
                ...
                BLL.Admin_Bll.Delete_applyborrowfrombook(bookid);
                BLL.Admin_Bll.Delete_commnetfrombook(bookid);
                if (BLL.Admin_Bll.Delete_Per(bookid))
                {
                    Response.Write("<script>alert('删除成功');location.href='admin_book_info.aspx'</script>");
                }
                else
                {
                    Response.Write("<script>alert('删除失败');location.href='admin_book_info.aspx'</script>");
                }
            }
    ...
    {
        ViewState["pageindex"] = 1;
        dt_ComInfo = BLL.Admin_Bll.Search_Goods(search_name.Text, search_teacher.Text, int.Parse(dp_type.SelectedValue));
        if (dt_ComInfo.Rows.Count == 0)   //查询结果为空，则放上所有搜索项
        {
            dt_ComInfo = BLL.Admin_Bll.Get_Per();
            txbnotice.Text = "未查询到符合条件的结果，下列结果为所有书籍";
        }
        else                           //查询到结果
        {
